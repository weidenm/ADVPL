...
//Filtrando as Notas Fiscais através da Carga - DAI - <TLM> Alterado por Renata Alves - 26/08/10

		If MV_PAR09 == 1
        
			If Empty(MV_PAR10) .or. Empty(MV_PAR11)
				MsgAlert("Favor preencher o código da Carga.")
			Return()
			Endif
		...
		Elseif MV_PAR09 == 2
			...
			If Empty(MV_PAR01) .or. Empty(MV_PAR02) .or. Empty(MV_PAR03)  //ALTERADO
				MsgAlert("Favor preencher número de Nota Fiscal e Série.")
			Return()
			Endif
	...
	While  !lBreak  .And. nLenarray >= 0
		...
		While !Eof() .And. !lBreak .And. ;
			...
			For nx:=1 To 20
				...
				aadd(Atail(aNotas),"")
				aadd(Atail(aNotas),"")
				If nCursor==Val(cNotaAte) .or. (cAliasSFX)->(Eof ())
					lBreak :=.T.
					nx:=20
				EndIF
				nCursor++
...
Local cMunic 	:= "" //alterado por Weiden 
...
// ***************************************************
// INFORMAÇÃO ADICIONAL - Nome + CPF do motorista   *
