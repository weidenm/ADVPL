...
Static Function GridRom()
	...
	AADD(aFields,{"Item" ,"PESO"    ,"" ,3 ,0, ,"๛","C","" ,"V",  ,  ,".F."} )
...
//Posicionamento de item de carregamento crescente
	...
	if nItem = 2  // 2= Chamada botใo "PROX ITEM"
...
nProxIt := val(TRC2->Z1_ITEM) //Busca item atual em aberto como refer๊ncia para o pr๓ximo
...
//Passamos agora todas as informa็๕es para o calculo das dimen็๕es://MsObjSize( aInfo, aObjects, Mantem Propor็ใo , Disposi็ใo Horizontal )
	...
	if Alltrim(cItemRom) = ""
		...
		Return
	EndIf
	//Caso existir tela aberta anteriormente ela ้ finalizada.
	if ! empty(oDlg2) 
	//	alert("fechou")
		oDlg2:end()
	endif
	//Se nใo existir pr๓ximo item fecha a tela de Carregamento.
...
//*****************************************************************************
	...
	@ 030, nColG1+020 SAY oLblConc PROMPT "CONCLUอDOS"   SIZE 060, 011 OF oDlg2 FONT oFont2 COLORS 0, 16777215 PIXEL
		...
		if SZ3->(found())  //.AND. if SZ3->Z3_ROMANEI==cNumRom //retirado verifica romaneio para funcionar fora do romaneio.
			...
			If  Trim(SZ3->Z3_ROMANEI) <> "" .or. SZ3->Z3_REG='D' //CASO FARDO FOR ENCONTRADO*
				...
				if nTipoRet = 1 .and. Z3_ROMANEI <> cNumRom
					Alert("Este fardo nใo pertence ao romaneio "+Alltrim(cNumRom) +"."+ ENTER +"Favor utilizar a rotina Devolu็ใo para retornar para estoque.")
					Return
				EndIF
...
Static Function MudaItRom()
	...
	if Alltrim(cPassW2)  == "ronso"
		...
		oDlgPwd2:End()
		oDlg2:End()
		TelaCarga(2) //Chama pr๓ximo item
